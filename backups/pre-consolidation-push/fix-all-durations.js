  ...
  try {
    ...
    if (contentLength && contentType && contentType.includes('audio')) {
      ...
      if (contentType.includes('mpeg') || contentType.includes('mp3')) {
        ...
        // Only return if it seems reasonable (30 seconds to 15 minutes)
        if (estimatedDuration >= 30 && estimatedDuration <= 900) {
          return estimatedDuration;
        }
      }
      
      // For M4A files, similar approach but different bitrate estimation
      if (contentType.includes('m4a') || contentType.includes('mp4')) {
        const estimatedDuration = Math.round(fileSizeKB * 0.055);
        if (estimatedDuration >= 30 && estimatedDuration <= 900) {
          return estimatedDuration;
        }
      }
...
async function fixAllDurations() {
  console.log('⏱️ Comprehensive Duration Fix - All 3:00 Placeholders');
  ...
  // Process more tracks - up to 200 to make a significant impact
  const targetCount = Math.min(placeholderDurationTracks.length, 200);
  console.log(`\n🔍 Processing up to ${targetCount} tracks in batches of 20...`);
  ...
  for (let i = 0; i < targetCount; i++) {
    ...
    if (processedCount % 20 === 1) {
      console.log(`\nBatch ${Math.ceil(processedCount / 20)}: Tracks ${processedCount}-${Math.min(processedCount + 19, targetCount)}`);
    }
    ...
    if (processedCount % 20 === 0) {
      console.log(`   📊 Batch ${Math.ceil(processedCount / 20)} complete: ${fixedCount} fixed out of ${processedCount} processed`);
      await new Promise(resolve => setTimeout(resolve, 3000)); // Longer pause between batches
    } else {
      await new Promise(resolve => setTimeout(resolve, 150)); // Faster within batches
    }
  }
  ...
  if (fixedCount > 0) {
    ...
    // Show improvement
    const originalPlaceholders = 528; // From previous count
    const improvement = originalPlaceholders - finalPlaceholder180;
    console.log(`   Improvement: ${improvement} tracks upgraded from 3:00 placeholder to real duration`);
  } else {
