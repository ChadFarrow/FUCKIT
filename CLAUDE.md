...
**Keysend capability** (`components/Lightning/BitcoinConnectProvider.tsx` ~line 329): read from WebLN `GetInfoResponse.methods` — NWC wallets populate it with `pay_keysend`/`multi_pay_keysend`, extensions with `keysend`. Hardcoded provider-type whitelist (`alby`/`alby-hub`/`extension`/`coinos`) is only used as a fallback when the wallet returns an empty `methods` array. Do NOT revert to type-whitelist-only — it falsely rejects Alby Hub connected via pasted NWC URL (`connectorType = 'nwc.generic'` classifies as type `'nwc'`, which the old whitelist excluded). Still do NOT probe with a real keysend — triggers a payment popup in Alby extension. `detectWalletProviderType()` in `lib/lightning/wallet-detection.ts` still drives Lightning-address inference, avatar lookup, and the fallback keysend signal.

Bitcoin Connect `connectorType` values (per `@getalby/bitcoin-connect/dist/connectors/index.d.ts`): `extension.generic`, `nwc.alby`, `nwc.albyhub`, `nwc.generic`, `nwc.coinos`, `nwc.primal`, `nwc.lnfi`, `nwc.flash`, `nwc.cashume`, `nwc.lnbits`, `nwc.rizful`, `lnbits`, `lnc`. Primal (`nwc.primal`) exposes `provider.keysend` via WebLN shim but its relay doesn't implement `pay_keysend` — the methods-array check correctly rejects it because Primal's NWC relay doesn't advertise `pay_keysend` in `get_info`.
