...
### Marking Feeds Dead (`Feed.markedDead`, admin "Mark Feed as Dead")
For feeds **taken down upstream** (removed from Wavlake, etc.). Unlike deleting — which lets the 4 AM Step 5b cron re-mint the feed from PI artist search — this keeps the row in the DB and flags it dead. `Feed.markedDead Boolean @default(false)` (migration `20260716000000_add_marked_dead_to_feed`).
...
- **Read paths filter `markedDead: false`**: `albums-fast` (grid + podcasts case), `feeds/recent` (both query sites), `search` (Prisma exact path **and** both raw-SQL fuzzy queries in `lib/fuzzy-search.ts` — `f."markedDead" = false`), `/api/albums` (`feedWhere`), and the `/publisher/[id]` page (all 5 album/artist queries). When adding a new feed-listing surface, add the filter there too.
...
- **Railway migrate gotcha (issue #122)**: the Dockerfile does NOT run migrations on deploy. After merging, run `railway run --service StableKraft --environment production npm run db:migrate` or every `markedDead`-filtered query 500s in prod (the column won't exist). The column is additive with a default, so applying it *before* the code deploys is safe.
