...
- **15-minute in-memory server cache** in `albums-fast/route.ts` — Railway redeploy clears it; manual bypass is `?refresh=true` query param. `POST /api/admin/clear-cache` does **not** reach this cache (or the 5-min `searchCache` in `app/api/search/route.ts:12`) — it only clears the `dataService` cache, despite what the name suggests. Verified 2026-04-19.
