...
**Signer nudge toast** (`lib/nostr/signer-nudge.ts`): `withSignerNudge()` wraps `signEvent`/`getPublicKey`, shows dismissable toast after **4s** ("Waiting on Primal to approve…"), hard-fails at **125s** (sits OUTSIDE the NIP-46 client's 120s relay-request timeout so the client's richer error surfaces first). `NIP46Signer.signEvent`/`getPublicKey` in `signer.ts` route through it automatically; direct `client.signEvent` callers in `LoginModal` also wrap manually. Throttled to 8s so bursts don't spam toasts. Pattern adapted from `soapbox-pub/ditto`.

**iOS PWA reconnect feedback**: `useNip46Connection`'s `visibilitychange` handler emits `toast.success('Signer reconnected')` on successful reconnect, or an actionable red toast with Retry on failure. No more silent hangs. `NostrProvider` (`contexts/NostrContext.tsx:88`) has its *own* `visibilitychange` handler that reconnects the NIP-46 relay on every page — the modal-scoped hook only fires while `LoginModal` is mounted, so the provider-scoped handler is what keeps boost/favorite working on `/favorites`, NowPlaying, etc. Do **not** delete it as "redundant".

**Reconnect ordering** (`lib/nostr/signer-reconnect.ts:317` `ensureSignerAvailable`, `:380-397` the fast-path): try `verifyNIP46Connection()` on the live in-memory NIP-46 client *before* `restoreNIP46Connection()` rebuilds from localStorage. The in-memory client has session state (signer app pubkey, bunker relays, rate-limit counters) that localStorage can't fully reproduce, and Safari ITP may have cleared storage entirely. `nip46-client.ts:authenticate()` also checks actual `WebSocket.readyState` via `relayManager.isConnected()` and force-reconnects dead sockets via `startRelayConnection()` — replacing an earlier passive 3s wait that never recovered. Do **not** invert this ordering or remove the explicit WebSocket-state check.

**Multi-relay bunker URIs** (`lib/nostr/nip46-client.ts:309` `bunkerRelays`, `:3276-3277` publish fan-out): bunker:// URIs with multiple `relay=` params are subscribed *and* published across all listed relays, not just the first. Survives one-relay blocks (e.g. Firefox blocks `relay.primal.net`). Secretless bunker URIs (Aegis-style) have a 25s fail-fast timeout since they require manual signer approval. Do **not** "simplify" back to first-relay-only.

**Pre-sign ping + manual reconnect** (`lib/nostr/nip46-client.ts` `pingSigner`, `lib/nostr/signer-reconnect.ts` `reconnectSignerManually`): our relay socket being alive does **not** prove the signer's own subscription is alive — if Primal's iOS app was killed overnight, `sign_event` is published into the void and the client waits the full 120s before failing. `BoostButton.tsx` pings the signer with a 5s timeout before calling `signEvent`; any response (including "unknown method") counts as reachable. On ping failure the boost fails fast with an error toast whose **Reconnect** action calls `reconnectSignerManually()` — tries in-memory revive via `verifyNIP46Connection`, falls back to `restoreNIP46Connection` from localStorage, **never touches the user record**. The same helper powers the "Reconnect signer" button in `NostrSettings` (next to Logout, NIP-46 logins only). Do **not** add the ping inside `signEvent` itself — the current location lets non-boost callers (publish queue, favorites) keep their existing behavior, and keeps the extra round-trip scoped to the one flow that most often hits the stale-Primal case.
...
**Card-menu UI** (pattern from `hzrd149/nostrudel`) — no tabs. Cards: Browser Extension (shown only if `window.nostr` detected), Bunker URI (paste `bunker://` / `nostrconnect://`), Amber (Android), Primal (QR code). `view` state: `'menu' | 'bunker' | 'primal' | 'amber'` (`LoginModal.tsx:30`). The old "More options (nostr-login)" card was removed in PR #98 — with `noBanner: true` the `<nl-auth>` element never mounts, so clicking it dismissed the modal with nothing rendered. `NostrLoginInit.tsx` is still mounted for session-restore of legacy nostr-login users.
...
**Profile backfill** (`contexts/NostrContext.tsx:174-185`): the login route (`app/api/nostr/auth/login/route.ts`) intentionally returns `displayName`/`avatar`/`bio`/`lightningAddress` as null to avoid a ~21s relay round-trip (login now completes in ~20ms). `NostrContext` auto-calls `refreshUser()` on mount whenever `user.displayName` is falsy — fetches the kind-0 profile from relays in the background, updates the DB row, and re-renders. Do **not** reintroduce the synchronous profile lookup in the login route; the 1000× perf delta was the whole point of PR #98. The login route also only updates `nostrNpub` on returning users so a fresh login can't wipe previously-fetched profile fields.
...
**Stale-signer recovery** (`lib/nostr/publish-queue.ts:101` `flushQueue`): routes through `ensureSignerAvailable()` from `signer-reconnect.ts:317` (same wrapper `BoostButton.tsx` uses) instead of a manual `isAvailable() + NIP-55-only` branch. Without this, a stale singleton signer (iOS WebSocket killed during backgrounding, page-mount race, first-flush after reload) silently dropped the favorite — boost was unaffected because it already gated on `ensureSignerAvailable`. Do **not** revert to the manual branch.

**Failure toasts** (`lib/nostr/publish-queue.ts` `flushQueue`): `ensureSignerAvailable` failure, zero-relay connectivity, and per-item sign/publish errors all emit `toast.error`/`toast.warning` instead of silently `resolve(null)`. Sign-failure toast includes a `Reconnect` action that calls `reconnectSignerManually()`. This was added after an Amber-on-Android session where "favorites work" reports masked a full-session failure (no prompt, no toast, queue silently swallowing every item). Do **not** remove the toasts or revert to silent resolve — the publish queue is the only place where a user-initiated Nostr write can fail invisibly, and diagnosing signer bugs without it is nearly impossible.
...
**Keysend capability** (`components/Lightning/BitcoinConnectProvider.tsx:336–357`): two signals combined with **OR**. Signal A is WebLN `GetInfoResponse.methods` — NWC wallets populate it with `pay_keysend`/`multi_pay_keysend`, extensions with `keysend`. Signal B is the provider-type whitelist (`alby`/`alby-hub`/`extension`/`coinos`) from `detectWalletProviderType()`. Either is sufficient. OR (not "methods overrides whitelist") is deliberate: methods-first falsely rejected Alby Hub users whose `get_info` returned a methods array lacking `pay_keysend` (older hub versions, partial NWC permissions, or stale cached responses). The OR still picks up Alby Hub via pasted `nwc.generic` URL (classified as type `'nwc'`, not in whitelist — rescued by Signal A) and still rejects Primal (type `'nwc'` AND methods lacks `pay_keysend` → both signals false). An eager `setKeysendSupported(...)` pass runs immediately after `detectWalletProviderType` and before `provider.getInfo()`, so the UI banner and lnaddress keysend-fallback gate don't flash `false` during the 1–5s NWC relay round-trip on cold start. Still do NOT probe with a real keysend — triggers a payment popup in Alby extension. `detectWalletProviderType()` in `lib/lightning/wallet-detection.ts` also drives Lightning-address inference and avatar lookup.

Bitcoin Connect `connectorType` values (per `@getalby/bitcoin-connect/dist/connectors/index.d.ts`): `extension.generic`, `nwc.alby`, `nwc.albyhub`, `nwc.generic`, `nwc.coinos`, `nwc.primal`, `nwc.lnfi`, `nwc.flash`, `nwc.cashume`, `nwc.lnbits`, `nwc.rizful`, `lnbits`, `lnc`. Primal (`nwc.primal`) exposes `provider.keysend` via WebLN shim but its relay doesn't implement `pay_keysend` — the OR-combined check correctly rejects it because Primal's type `'nwc'` is not in the whitelist and its `get_info` methods array doesn't advertise `pay_keysend`.

**Wallet/Nostr are independent** (`components/Lightning/BitcoinConnectProvider.tsx`): Nostr logout does **not** disconnect the Lightning wallet. The prior behavior (wipe wallet + set `wallet_manually_disconnected=true` on every Nostr logout) forced a manual wallet re-pair every time a user logged out of Nostr to reseat a broken NIP-46 signer. The two auth surfaces live in different scopes and should stay that way. The remaining Nostr→wallet interactions (`:187-231` auto-pick-up Alby WebLN on NIP-07 login; `:113-138` `wallet_restore_after_login` Android fix) are *restorative*, not destructive — leave them alone.
