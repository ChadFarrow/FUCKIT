...
- **15-minute in-memory server cache** in `albums-fast/route.ts` — Railway redeploy clears it; manual bypass is `?refresh=true` query param. Cache state now lives in `lib/caches/albums-fast-cache.ts` so `POST`/`PUT`/`DELETE /api/feeds` auto-invalidate it (via `invalidateAlbumsFastCache()`); the twin 5-min `searchCache` (`lib/caches/search-cache.ts`) is invalidated on the same hooks. This means a podping-minted feed or admin edit shows up on the very next grid/search request without waiting the TTL. `POST /api/admin/clear-cache` still does **not** reach either cache (it only clears the `dataService` cache); reach for `?refresh=true` or a feed mutation if you need a forced bust. Originally verified 2026-04-19; auto-invalidation added 2026-04-21 (issue #110).
