...
- **Manual Offline mode** (`DownloadsContext`): `isOnline` is driven **solely** by a persisted user toggle (`localStorage['sk_offline_mode']`) on the /downloads page — the app deliberately does NOT react to real `navigator.onLine` (connection loss must never force offline UI or redirect; playback prefers the downloaded blob regardless). `isOnline` only gates starting **new** downloads. Do not re-add `useOnlineStatus` reactivity.
