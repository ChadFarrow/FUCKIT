...
**Signer nudge toast** (`lib/nostr/signer-nudge.ts`): `withSignerNudge()` wraps `signEvent`/`getPublicKey`, shows dismissable toast after **4s**, hard-fails at **125s** (sits outside the 120s NIP-46 client timeout so the client's richer error fires first). `NIP46Signer.signEvent`/`getPublicKey` in `signer.ts` route through it automatically; direct `client.signEvent` callers in `LoginModal` also wrap manually. Throttled to 8s so bursts don't spam toasts. Pass `kind: event.kind` for kind-aware labels ("Waiting on Primal to approve favorites update…" instead of generic "signing"); see `KIND_LABELS` map. Pass `isBunkerConnected: () => client.isConnected()` so when the bunker relay is dead (iOS Safari killed the WebSocket) the toast becomes an orange "Primal relay unreachable — check your connection" warning instead of a misleading "Waiting…". Pattern adapted from `soapbox-pub/ditto`.
