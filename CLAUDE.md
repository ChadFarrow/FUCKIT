...
**Henrik Flyman artist-scoped Wavlake rule** (`isHenrikFlymanWavlakeMirror` in `lib/feed-exclusions.ts`): Wavlake keeps minting brand-new mirror feeds for Henrik (2026 releases like Unbreakable / Is This The End / The Writing's on the Wall / They Intend to Destroy Beauty) faster than their `/feed/music/<uuid>` URLs can be added to `BLACKLISTED_FEED_URLS`. Since he pulled **all** his music off Wavlake, `isNotBlacklistedFeed` also drops any `wavlake.com` feed whose artist is "Henrik Flyman" — current and future. This is the **one sanctioned exception** to the no-platform-filter rule, and it's scoped to a single artist by name; do NOT generalize it into a broader "hide Wavlake" filter.
