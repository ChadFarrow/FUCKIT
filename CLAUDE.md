...
The **native** counterpart to the battery hint above: the zapstore app is a Capacitor WebView, and without a foreground service GrapheneOS/aggressive Android suspends the backgrounded WebView process within seconds of locking → audio dies. This pins the process **and holds a partial wake lock**. Shipped in zapstore **v1.1 (`versionCode 2`)** with the FGS; **v1.2 (`versionCode 3`)** added the wake lock — v1.0.0 had no service and died when locked.

- **Wake lock is not optional (v1.2 fix):** a foreground service only stops the process from being *killed* — it does **not** stop the CPU from entering deep sleep during Doze, which freezes the WebView's audio decode AND the JS timers that drive the ping-pong track transitions. v1.1 shipped FGS-only and still died on GrapheneOS with the screen locked. `PlaybackKeepAliveService` now acquires a `PARTIAL_WAKE_LOCK` (tag `StableKraft::PlaybackKeepAlive`, `setReferenceCounted(false)`) in `onStartCommand` and releases it in `onDestroy` (both wrapped in try/catch — wake-lock failure degrades to FGS-only, never crashes the service). Screen/keyboard still turn off; only the CPU stays awake. The `WAKE_LOCK` permission was already declared in the manifest since v1.1 but was unused until now. Do **not** add a timeout to the lock — playback is long-lived; it's released deterministically when playback stops → `stop()` → `stopService` → `onDestroy`.
