...
Body `{ dryRun?, limit? }` — `dryRun:true` returns `wouldHide` and writes nothing. On write it invalidates albums-fast + search caches (like `mark-dead`). Reversible via the "Hide Feed" card's Restore. **Admin UI:** `/admin` → "Check for Dead Feeds" card — Preview (dryRun) then "Hide N confirmed-dead feeds". **Cron:** `.github/workflows/check-dead-feeds.yml`, weekly (Mon 10:00 UTC) + `workflow_dispatch`, runs with auto-hide and logs the JSON report as the audit trail. Gated by the `/api/admin/:path*` matcher (Bearer `ADMIN_SECRET`).
