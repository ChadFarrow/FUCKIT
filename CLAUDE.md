...
- **Read paths filter `markedDead: false`**: `albums-fast` (grid + podcasts case), `feeds/recent` (both query sites), `search` (Prisma albums exact path, the podcast-search query, the raw-SQL artist exact-match fallback — `f."markedDead" = false`, **and** both raw-SQL fuzzy queries in `lib/fuzzy-search.ts`), `/api/albums` (`feedWhere`), the `/publisher/[id]` page (all 5 album/artist queries), `/api/publishers` + `/api/publishers/[id]` (publisher grid + detail-API, both queries each), `/api/parsed-feeds` (core data loader via `lib/data-service.ts`), and `/api/feeds/opml`. When adding a new feed-listing surface, add the filter there too. **Out of scope on purpose**: track-level surfaces (a hidden feed's individual tracks still surface in track search — `lib/fuzzy-search.ts` track query, `tracks/search`) since the flag hides *feeds*, not tracks.
