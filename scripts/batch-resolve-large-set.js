...
async function resolveRemoteItem(feedGuid, itemGuid) {
  ...
  try {
    ...
    console.log(`🔍 Resolving: ${feedGuid} / ${itemGuid}`);
    ...
    if (!data.episode) {
      console.log(`❌ No episode found for: ${feedGuid} / ${itemGuid}`);
      return null;
    }
    ...
    console.log(`✅ Resolved: "${track.title}" by ${track.artist} (${track.album})`);
    return track;
...
async function removeDuplicates() {
  try {
    ...
    // Create a map to track unique tracks by feedGuid + itemGuid
    ...
    if (duplicates.length > 0) {
      console.log(`🧹 Removing ${duplicates.length} duplicate tracks...`);
      data.musicTracks = Array.from(uniqueTracks.values());
      
      // Update metadata
      data.metadata.totalTracks = data.musicTracks.length;
      data.metadata.lastUpdated = new Date().toISOString();
      
      fs.writeFileSync(dbPath, JSON.stringify(data, null, 2));
      console.log(`✅ Removed duplicates, ${data.musicTracks.length} unique tracks remain`);
    } else {
...
async function main() {
  console.log(`🚀 Starting batch resolution of ${remoteItems.length} remote items...`);
  ...
  for (let i = 0; i < remoteItems.length; i++) {
    ...
    try {
      console.log(`\n[${i + 1}/${remoteItems.length}] Processing...`);
      
      const track = await resolveRemoteItem(item.feedGuid, item.itemGuid);
