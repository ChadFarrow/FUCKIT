...
// Test with first few items to debug the 400 errors
async function main() {
  console.log(`🚀 Testing first 5 remote items to debug 400 errors...`);
  ...
  try {
    ...
    const testResponse = await fetch('https://api.podcastindex.org/api/1.0/stats/current', { headers });
    if (!testResponse.ok) {
  ...
  for (let i = 0; i < remoteItems.length; i += BATCH_SIZE) {
    ...
    console.log(`\n📦 Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(remoteItems.length / BATCH_SIZE)}`);
    ...
    for (const result of results) {
      if (result.status === 'fulfilled' && result.value) {
        try {
          ...
          console.error('❌ Failed to save track:', saveError.message);
          failed++;
        }
      } else {
        failed++;
