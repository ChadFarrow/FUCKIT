...
async function getMediaUrls(track, index, total) {
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 300)); // Reduced delay between API calls
...
async function main() {
  console.log('🚀 HGH Media URL Resolution Script (FAST MODE)');
  ...
  // Process in larger batches with shorter delay
...
`;
    ...
    if (i + batchSize < tracksNeedingUrls.length) {
      console.log(`⏳ Waiting 1.5 seconds before next batch...`); // Reduced from 3 seconds
