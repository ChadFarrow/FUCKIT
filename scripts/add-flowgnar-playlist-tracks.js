...
/**
 * Add Flowgnar Music Playlist tracks to the database
 * This script processes remote items from the Flowgnar playlist XML
 */
...
async function addFlowgnarPlaylistTracks() {
  try {
    console.log('🎵 Adding Flowgnar Music Playlist tracks...\n');
    
    // Download the Flowgnar playlist XML
    console.log('📥 Downloading Flowgnar playlist XML...');
    const response = await fetch('https://raw.githubusercontent.com/ChadFarrow/chadf-musicl-playlists/refs/heads/main/docs/flowgnar-music-playlist.xml');
    ...
    console.log(`Found ${remoteItems.length} remote items in Flowgnar playlist`);
    ...
    for (const item of remoteItems) {
      ...
      if (existingTrack) {
        // Update the source to include Flowgnar if not already present
        if (!existingTrack.source?.includes('Flowgnar')) {
          const currentSource = existingTrack.source || '';
          existingTrack.source = currentSource ? `${currentSource}, Flowgnar` : 'Flowgnar';
    ...
    // Print summary
    console.log('\n' + '='.repeat(50));
    console.log('📊 FLOWGNAR IMPORT SUMMARY:');
    ...
    console.log('\n💡 Next steps:');
    console.log('   Run: node scripts/resolve-flowgnar-tracks.js');
    console.log('   This will resolve the track metadata using the Podcast Index API');
    
  } catch (error) {
    console.error('❌ Error adding Flowgnar tracks:', error);
    process.exit(1);
  }
}
...
if (require.main === module) {
  addFlowgnarPlaylistTracks();
}

module.exports = { addFlowgnarPlaylistTracks };