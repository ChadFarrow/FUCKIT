...
async function fetchIAMPlaylist() {
    console.log('🎵 Fetching IAM Music Playlist from GitHub...\n');
    
    const feedUrl = 'https://raw.githubusercontent.com/ChadFarrow/chadf-musicl-playlists/refs/heads/main/docs/IAM-music-playlist.xml';
        ...
        remoteItemsList.forEach((remoteItem, index) => {
            if (remoteItem) {
                const processedItem = {
                    ...
                    // Source info
                    source: 'IAM Music Playlist',
                    addedFrom: 'GitHub - ChadFarrow/chadf-musicl-playlists',
                    playlistTitle: channel.title || 'IAM Playlist'
                };
...
async function main() {
    try {
        console.log('🚀 Starting IAM Music Playlist Import\n');
        console.log('=' .repeat(50) + '\n');
        
        // Fetch the XML feed
        const xmlData = await fetchIAMPlaylist();
