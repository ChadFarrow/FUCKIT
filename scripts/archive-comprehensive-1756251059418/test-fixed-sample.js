#!/usr/bin/env node

const fs = require('fs');
const path = require('path');
const crypto = require('crypto');

// Load environment variables from .env.local
const envPath = path.join(__dirname, '../.env.local');
if (fs.existsSync(envPath)) {
  const envContent = fs.readFileSync(envPath, 'utf8');
  envContent.split('\n').forEach(line => {
    const [key, ...valueParts] = line.split('=');
    if (key && valueParts.length > 0) {
      process.env[key.trim()] = valueParts.join('=').trim();
    }
  });
}

const API_KEY = process.env.PODCAST_INDEX_API_KEY;
const API_SECRET = process.env.PODCAST_INDEX_API_SECRET;

// Read remote items from file
function parseRemoteItemsFromFile(filePath) {
  const content = fs.readFileSync(filePath, 'utf8');
  const lines = content.split('\n').filter(line => line.trim());
  
  const remoteItems = [];
  
  for (const line of lines) {
    const feedGuidMatch = line.match(/feedGuid="([^"]+)"/);
    const itemGuidMatch = line.match(/itemGuid="([^"]+)"/);
    
    if (feedGuidMatch && itemGuidMatch) {
      remoteItems.push({
        feedGuid: feedGuidMatch[1],
        itemGuid: itemGuidMatch[1]
      });
    }
  }
  
  return remoteItems;
}

// Podcast Index API authentication
function createAuthHeaders() {
  const unixTime = Math.floor(Date.now() / 1000);
  const data4Hash = API_KEY + API_SECRET + unixTime;
  const hash = crypto.createHash('sha1').update(data4Hash).digest('hex');
  
  return {
    'X-Auth-Date': unixTime.toString(),
    'X-Auth-Key': API_KEY,
    'Authorization': hash,
    'User-Agent': 'FUCKIT-Music-App/1.0'
  };
}

// Cache for feed lookups to avoid duplicate API calls
const feedCache = new Map();

// Get feed ID from GUID
async function getFeedId(feedGuid) {
  if (feedCache.has(feedGuid)) {
    return feedCache.get(feedGuid);
  }
  
  try {
    const headers = createAuthHeaders();
    const url = `https://api.podcastindex.org/api/1.0/podcasts/byguid?guid=${encodeURIComponent(feedGuid)}`;
    
    const response = await fetch(url, { headers });
    const data = await response.json();
    
    let result;
    if (data.status === 'true' && data.feed && data.feed.id) {
      result = { 
        feedId: data.feed.id, 
        feedTitle: data.feed.title,
        feedUrl: data.feed.url,
        feedImage: data.feed.image
      };
    } else {
      result = { error: 'Feed not found' };
    }
    
    feedCache.set(feedGuid, result);
    return result;
  } catch (error) {
    const result = { error: error.message };
    feedCache.set(feedGuid, result);
    return result;
  }
}

// Get episode from feed using feedId and episode GUID
async function getEpisodeFromFeed(feedId, itemGuid) {
  try {
    const headers = createAuthHeaders();
    const url = `https://api.podcastindex.org/api/1.0/episodes/byfeedid?id=${feedId}&max=100`;
    
    const response = await fetch(url, { headers });
    const data = await response.json();
    
    if (data.status === 'true' && data.items) {
      // Find the episode with matching GUID
      const episode = data.items.find(ep => ep.guid === itemGuid);
      if (episode) {
        return { episode };
      }
    }
    
    return { error: 'Episode not found in feed' };
  } catch (error) {
    return { error: error.message };
  }
}

// Main resolution function
async function resolveRemoteItemFixed(feedGuid, itemGuid) {
  try {
    // Step 1: Get feed ID
    const feedResult = await getFeedId(feedGuid);
    if (feedResult.error) {
      return { error: `Feed lookup failed: ${feedResult.error}` };
    }
    
    // Step 2: Get episode from feed
    const episodeResult = await getEpisodeFromFeed(feedResult.feedId, itemGuid);
    if (episodeResult.error) {
      return { error: `Episode lookup failed: ${episodeResult.error}` };
    }
    
    // Add feed info to episode
    const episode = episodeResult.episode;
    episode.feedGuid = feedGuid;
    episode.feedUrl = feedResult.feedUrl;
    episode.feedTitle = feedResult.feedTitle;
    episode.feedImage = feedResult.feedImage;
    
    return { episode };
    
  } catch (error) {
    return { error: error.message };
  }
}

// Load existing music tracks for duplicate checking
const musicTracksPath = path.join(__dirname, '../data/music-tracks.json');
let musicData = { musicTracks: [], episodes: [], feeds: [] };

if (fs.existsSync(musicTracksPath)) {
  try {
    musicData = JSON.parse(fs.readFileSync(musicTracksPath, 'utf8'));
  } catch (error) {
    console.error('Error reading existing music tracks:', error.message);
  }
}

// Check if track already exists
function isTrackDuplicate(episode) {
  return musicData.musicTracks.some(track => 
    track.feedGuid === episode.feedGuid && track.episodeGuid === episode.guid
  );
}

// Test with larger sample
async function testLargerSample() {
  const inputFile = process.argv[2] || '../batch-items-308.txt';
  const sampleSize = parseInt(process.argv[3]) || 30;
  
  if (!fs.existsSync(inputFile)) {
    console.error(`File not found: ${inputFile}`);
    process.exit(1);
  }
  
  const remoteItems = parseRemoteItemsFromFile(inputFile);
  console.log(`🧪 Testing FIXED approach with ${sampleSize} items from ${remoteItems.length} total...\n`);
  
  // Take items from different positions for better sampling
  const testItems = [];
  const step = Math.floor(remoteItems.length / sampleSize);
  for (let i = 0; i < sampleSize && i * step < remoteItems.length; i++) {
    testItems.push(remoteItems[i * step]);
  }
  
  console.log(`📊 Sample strategy: Every ${step}th item for representative coverage\n`);
  
  let successCount = 0;
  let duplicateCount = 0;
  let failureCount = 0;
  const uniqueFeeds = new Set();
  const trackDetails = [];
  
  for (let i = 0; i < testItems.length; i++) {
    const { feedGuid, itemGuid } = testItems[i];
    const testIndex = i + 1;
    
    console.log(`📡 [${testIndex}/${testItems.length}] Testing: ${feedGuid.substring(0, 8)}...`);
    
    const result = await resolveRemoteItemFixed(feedGuid, itemGuid);
    
    if (result.error) {
      console.log(`   ❌ Failed: ${result.error}`);
      failureCount++;
    } else {
      uniqueFeeds.add(result.episode.feedTitle);
      
      // Check for duplicates
      if (isTrackDuplicate(result.episode)) {
        console.log(`   ⏭️  DUPLICATE: "${result.episode.title}"`);
        duplicateCount++;
      } else {
        console.log(`   ✅ SUCCESS: "${result.episode.title}"`);
        console.log(`   📚 Feed: "${result.episode.feedTitle}"`);
        console.log(`   🎨 Artwork: ${result.episode.image ? 'Yes' : 'No'}`);
        console.log(`   🔗 Audio: ${result.episode.enclosureUrl ? 'Yes' : 'No'}`);
        console.log(`   📊 Duration: ${result.episode.duration || 0}s`);
        
        trackDetails.push({
          title: result.episode.title,
          feed: result.episode.feedTitle,
          duration: result.episode.duration || 0,
          hasArtwork: !!result.episode.image,
          hasAudio: !!result.episode.enclosureUrl
        });
        
        successCount++;
      }
    }
    
    console.log('');
    
    // Small delay between requests
    await new Promise(resolve => setTimeout(resolve, 200));
  }
  
  // Summary Report
  console.log('=' .repeat(60));
  console.log('🎯 LARGE SAMPLE TEST RESULTS');
  console.log('=' .repeat(60));
  console.log(`📊 Sample Size: ${testItems.length}/${remoteItems.length} items`);
  console.log(`✅ Successful: ${successCount} (${((successCount/testItems.length)*100).toFixed(1)}%)`);
  console.log(`⏭️  Duplicates: ${duplicateCount} (${((duplicateCount/testItems.length)*100).toFixed(1)}%)`);
  console.log(`❌ Failed: ${failureCount} (${((failureCount/testItems.length)*100).toFixed(1)}%)`);
  console.log(`🎵 Unique Feeds: ${uniqueFeeds.size}`);
  
  if (uniqueFeeds.size > 0) {
    console.log('\n📚 DISCOVERED FEEDS:');
    Array.from(uniqueFeeds).sort().forEach((feed, i) => {
      console.log(`   ${i+1}. ${feed}`);
    });
  }
  
  if (trackDetails.length > 0) {
    console.log('\n🎵 SAMPLE TRACKS ANALYSIS:');
    const withArtwork = trackDetails.filter(t => t.hasArtwork).length;
    const withAudio = trackDetails.filter(t => t.hasAudio).length;
    const avgDuration = trackDetails.reduce((sum, t) => sum + t.duration, 0) / trackDetails.length;
    
    console.log(`   🎨 Tracks with artwork: ${withArtwork}/${trackDetails.length} (${((withArtwork/trackDetails.length)*100).toFixed(1)}%)`);
    console.log(`   🔗 Tracks with audio: ${withAudio}/${trackDetails.length} (${((withAudio/trackDetails.length)*100).toFixed(1)}%)`);
    console.log(`   ⏱️  Average duration: ${Math.round(avgDuration)}s (${(avgDuration/60).toFixed(1)} min)`);
    
    console.log('\n🎼 SAMPLE TRACKS:');
    trackDetails.slice(0, 10).forEach((track, i) => {
      const artIcon = track.hasArtwork ? '🎨' : '📷';
      const audioIcon = track.hasAudio ? '🔗' : '❌';
      console.log(`   ${i+1}. ${artIcon}${audioIcon} "${track.title}" (${track.feed}) - ${Math.round(track.duration/60)}min`);
    });
    
    if (trackDetails.length > 10) {
      console.log(`   ... and ${trackDetails.length - 10} more tracks`);
    }
  }
  
  // Recommendations
  console.log('\n💡 RECOMMENDATIONS:');
  if (successCount / testItems.length > 0.8) {
    console.log('   ✅ High success rate! Safe to process full batch.');
  } else if (successCount / testItems.length > 0.5) {
    console.log('   ⚠️  Moderate success rate. Consider investigating failures.');
  } else {
    console.log('   ❌ Low success rate. Review approach before full batch.');
  }
  
  if (duplicateCount > successCount) {
    console.log('   ⚠️  Many duplicates found. Consider checking existing database.');
  }
  
  console.log(`\n🚀 Ready to process all ${remoteItems.length} items? Success rate: ${((successCount/testItems.length)*100).toFixed(1)}%`);
}

testLargerSample().catch(console.error);