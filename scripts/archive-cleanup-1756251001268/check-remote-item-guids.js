...
// The GUIDs from the user's podcast:remoteItem list
const remoteItemGuids = [
  ...
  { feedGuid: "69c634ad-afea-5826-ad9a-8e1f06d6470b", itemGuid: "2b8cef29-6aa5-4f2f-a726-92f916f2424a" },
  { feedGuid: "b39fc3b2-2742-5944-a800-e87c42a093a3", itemGuid: "2fabcf8b-2c9a-4a0a-b895-3274043e5013" },
  { feedGuid: "999c4870-396c-5a6b-b22e-4c05ee46127d", itemGuid: "3f239db2-6c3c-4164-86c0-92c84f5caafa" },
  ...
  { feedGuid: "2b62ef49-fcff-523c-b81a-0a7dde2b0609", itemGuid: "2f1fabc9-54c0-4d96-93d8-98737356b448" },
...
async function checkRemoteItemGuids() {
  try {
    console.log('🔍 Checking for podcast:remoteItem GUIDs in music-tracks.json...\n');
    
    // Read the music tracks database
    const musicTracksPath = path.join(__dirname, '..', 'data', 'music-tracks.json');
    const musicData = JSON.parse(fs.readFileSync(musicTracksPath, 'utf8'));
    const musicTracks = musicData.musicTracks || musicData;
    
    console.log(`📊 Total tracks in database: ${musicTracks.length}\n`);
    ...
    // Check each remote item GUID
    for (const remoteItem of remoteItemGuids) {
      const { feedGuid, itemGuid } = remoteItem;
      
      // Look for exact matches
      const exactMatch = musicTracks.find(track => 
        track.feedGuid === feedGuid && 
        track.itemGuid && 
        (track.itemGuid._ === itemGuid || track.itemGuid === itemGuid)
      );
      ...
      if (exactMatch) {
        ...
        const feedGuidMatch = musicTracks.find(track => track.feedGuid === feedGuid);
        const itemGuidMatch = musicTracks.find(track => 
          track.itemGuid && 
          (track.itemGuid._ === itemGuid || track.itemGuid === itemGuid)
        );
        
        if (feedGuidMatch) {
          results.feedGuidMatches.push({
            remoteItem,
            track: feedGuidMatch
          });
        }
        ...
        if (!feedGuidMatch && !itemGuidMatch) {
          results.notFound.push(remoteItem);
        }
      }
    }
    ...
    console.log(`✅ Exact matches found: ${results.found.length}`);
    console.log(`🔍 Feed GUID matches only: ${results.feedGuidMatches.length}`);
    console.log(`🔍 Item GUID matches only: ${results.itemGuidMatches.length}`);
    console.log(`❌ Not found at all: ${results.notFound.length}\n`);
    
    if (results.found.length > 0) {
      console.log('🎯 EXACT MATCHES:');
      results.found.forEach((result, index) => {
        const track = result.track;
        console.log(`  ${index + 1}. Feed: ${result.remoteItem.feedGuid}`);
        console.log(`     Item: ${result.remoteItem.itemGuid}`);
        console.log(`     Title: ${track.title || 'N/A'}`);
        console.log(`     Artist: ${track.artist || track.feedArtist || 'N/A'}`);
        console.log(`     Publisher: ${track.publisher || 'N/A'}`);
        console.log('');
      });
    }
    
    if (results.feedGuidMatches.length > 0) {
      console.log('🔍 FEED GUID MATCHES ONLY:');
      results.feedGuidMatches.forEach((result, index) => {
        const track = result.track;
        console.log(`  ${index + 1}. Feed: ${result.remoteItem.feedGuid}`);
        console.log(`     Expected Item: ${result.remoteItem.itemGuid}`);
        console.log(`     Found Item: ${track.itemGuid?._ || track.itemGuid || 'N/A'}`);
        console.log(`     Title: ${track.title || 'N/A'}`);
        console.log(`     Artist: ${track.artist || track.feedArtist || 'N/A'}`);
        console.log('');
      });
    ...
    if (results.notFound.length > 0) {
      ...
      results.notFound.forEach((remoteItem, index) => {
        console.log(`  ${index + 1}. Feed: ${remoteItem.feedGuid}`);
        console.log(`     Item: ${remoteItem.itemGuid}`);
        console.log('');
      });
    }
    
    // Save detailed report
    const reportPath = path.join(__dirname, '..', 'data', 'remote-item-guid-check-report.json');
    const report = {
      timestamp: new Date().toISOString(),
      totalRemoteItems: remoteItemGuids.length,
      results: {
        exactMatches: results.found.length,
        feedGuidMatchesOnly: results.feedGuidMatches.length,
        itemGuidMatchesOnly: results.itemGuidMatches.length,
        notFound: results.notFound.length
      },
      details: {
        found: results.found,
        feedGuidMatches: results.feedGuidMatches,
        itemGuidMatches: results.itemGuidMatches,
    ...
    console.error('❌ Error checking remote item GUIDs:', error);
  }
}

checkRemoteItemGuids();
