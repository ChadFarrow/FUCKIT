...
/**
 * Add Mutton, Mead & Music Playlist tracks to the database
 * This script processes remote items from the MMM playlist XML
 */
...
async function addMMMPlaylistTracks() {
  try {
    console.log('🎵 Adding Mutton, Mead & Music Playlist tracks...\n');
    
    // Download the MMM playlist XML
    console.log('📥 Downloading Mutton, Mead & Music playlist XML...');
    const response = await fetch('https://raw.githubusercontent.com/ChadFarrow/chadf-musicl-playlists/refs/heads/main/docs/MMM-music-playlist.xml');
    ...
    for (const item of remoteItems) {
      ...
      if (existingTrack) {
        // Update the source to include MMM if not already present
        if (!existingTrack.source?.includes('MMM')) {
          const currentSource = existingTrack.source || '';
          existingTrack.source = currentSource ? `${currentSource}, MMM` : 'MMM';
    ...
    // Print summary
    console.log('\n' + '='.repeat(50));
    console.log('📊 MMM IMPORT SUMMARY:');
    ...
    console.log('\n💡 Next steps:');
    console.log('   Run: node scripts/resolve-mmm-tracks.js');
    console.log('   This will resolve the track metadata using the Podcast Index API');
    
  } catch (error) {
    console.error('❌ Error adding MMM tracks:', error);
    process.exit(1);
  }
}
...
if (require.main === module) {
  addMMMPlaylistTracks();
}

module.exports = { addMMMPlaylistTracks };